...
import static uk.gov.justice.cps.ApplicationRequestNotificationSentToCpsResponseTransformer.APPLICATION_REQUEST_NOTIFICATION_SENT_TO_CPS_RESPONSE_TRANSFORMER;
...
public class ApplicationRequestNotificationSentToCpsResponseTransformerTest {

    private static final String CPS_NOTIFICATION_PATH = "/CP/v1/notification/appliication-request-notification";

    @Rule
    public WireMockRule wireMockRule = new WireMockRule(options().port(8888).extensions(ApplicationRequestNotificationSentToCpsResponseTransformer.class));
    ...
    public void setupTest() throws IOException {
        ...
        wireMockRule.stubFor(post(urlPathEqualTo(CPS_NOTIFICATION_PATH))
                .willReturn(aResponse()
                        .withTransformer(APPLICATION_REQUEST_NOTIFICATION_SENT_TO_CPS_RESPONSE_TRANSFORMER, null, null)
                ));
    }
    ...
    public void shouldReturnValidResponseForMultipartRequest() throws IOException {
        ...
        mdo.addFormData("Notification", FileUtils.openInputStream(new File("src/test/resources/sample/application-request-notification.json")),
                MediaType.APPLICATION_JSON_TYPE, "application-request-notification.json");
    ...
    private JsonObject getPayload(final String applicationId) {
        ...
        return createObjectBuilder().add("applicationRequestNotification", applicationNotification).build();
    }
