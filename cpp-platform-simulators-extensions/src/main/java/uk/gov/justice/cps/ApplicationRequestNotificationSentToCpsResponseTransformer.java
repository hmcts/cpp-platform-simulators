...
public class ApplicationRequestNotificationSentToCpsResponseTransformer extends ResponseDefinitionTransformer {

    public static final Logger LOGGER = LoggerFactory.getLogger(ApplicationRequestNotificationSentToCpsResponseTransformer.class);

    public static final String APPLICATION_REQUEST_NOTIFICATION_SENT_TO_CPS_RESPONSE_TRANSFORMER = "application-request-notification-sent-to-cps-response-transformer";
    ...
    private void processRequestPayload(final Request request) {
        ...
        createFile(CPS_DIR, getFilename(requestParts.getApplicationId(), "payload", "txt"), requestParts.getMetadataPayload());
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (bodyPart.getHeader("Content-Disposition")[0].contains("Notification")) {
                    ...
                    requestParts.setApplicationId(extractApplicationId(content));
    ...
    public String getName() {
        return APPLICATION_REQUEST_NOTIFICATION_SENT_TO_CPS_RESPONSE_TRANSFORMER;
    }
    ...
    private String extractApplicationId(final String jsonPayload) {
        final JsonObject payloadAsJsonObject = createReader(new StringReader(jsonPayload)).readObject();
        return payloadAsJsonObject.getJsonObject("applicationRequestNotification").getString("applicationId");
    }
}
